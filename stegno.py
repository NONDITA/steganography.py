...
class Stegno:
    ...
    def main(self,root):
        ...
        b_encode = Button(f,text="Encode",bg='#fd7558',command= lambda :self.frame1_encode(f), padx=14)
        b_encode.config(font=('courier',14))
        b_decode = Button(f, text="Decode",bg='#fd7558',padx=14,command=lambda :self.frame1_decode(f))
    ...
    def frame1_decode(self,f):
        ...
        l1.grid()
        bws_button = Button(d_f2, text='Select',bg='#fd7558', command=lambda :self.frame2_decode(d_f2))
        bws_button.config(font=('courier',18))
        bws_button.grid()
        back_button = Button(d_f2, text='Cancel',bg='#fd7558', command=lambda : Stegno.home(self,d_f2))
    ...
    def frame2_decode(self,d_f2):
        ...
        if not myfile:
            ...
            l2.grid(pady=10)
            text_area = Text(d_f3, width=50, height=10,bg='#80c1ff')
            text_area.insert(INSERT, hidden_data)
            text_area.configure(state='disabled')
            text_area.grid()
            back_button = Button(d_f3, text='Cancel',bg='#fd7558', command= lambda :self.page3(d_f3))
            back_button.config(font=('courier',11))
            back_button.grid(pady=15)
            back_button.grid()
            show_info = Button(d_f3,text='More Info',bg='#fd7558',command=self.info)
    ...
    def frame1_encode(self,f):
        ...
        bws_button = Button(f2,text='Select',bg='#fd7558',command=lambda : self.frame2_encode(f2))
        bws_button.config(font=('courier',18))
        bws_button.grid()
        back_button = Button(f2, text='Cancel',bg='#fd7558', command=lambda : Stegno.home(self,f2))
        back_button.config(font=('courier',18))
        back_button.grid(pady=15)
        back_button.grid()
        f2.grid()
    ...
    def frame2_encode(self,f2):
        ...
        if not myfile:
            ...
            l2.config(font=('courier',18))
            l2.grid(pady=15)
            text_area = Text(ep, width=50, height=10,bg='#80c1ff')
            text_area.grid()
            encode_button = Button(ep, text='Cancel',bg='#fd7558', command=lambda : Stegno.home(self,ep))
            encode_button.config(font=('courier',11))
            data = text_area.get("1.0", "end-1c")
            back_button = Button(ep, text='Encode',bg='#fd7558', command=lambda : [self.enc_fun(text_area,myimg),Stegno.home(self,ep)])
            back_button.config(font=('courier',11))
            back_button.grid(pady=15)
            encode_button.grid()
            ep.grid(row=1)
    ...
    def enc_fun(self,text_area,myimg):
        ...
        if (len(data) == 0):
            ...
            messagebox.showinfo("Success","Encoding Successful\nFile is saved as png image in the same directory")
